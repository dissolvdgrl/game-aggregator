<div class="relative" x-data="{ isVisible: true }" @click.away="isVisible = false">
    <input
        ...
        type="text"
        class="bg-gray-800 pl-8 text-sm rounded-full focus:outline-none focus:shadow-outline w-64 px-3 py-1" placeholder="Search (Press '/' to focus)"
        x-ref="search"
        @keydown.window = "
        ...
        <ul>
            @foreach ($searchResults as $game)
                <li class="border-b border-gray-700">
                    <a
                        href="{{ route('games.show', $game['slug']) }}" class="block hover:bg-gray-700 flex items-center transition ease-in-out duration-150 p-3"
                        @if ($loop->last)
